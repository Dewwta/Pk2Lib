...
Reads and writes JoyMax PK2 archives, the package format used by Silkroad Online to store its game data. Handles the Blowfish-encrypted directory blocks and entry table, and can also decode the DDJ textures (DDS wrapped in a JoyMax header) used for item/skill icons into raw RGBA pixels.

Two implementations, same format, written independently:

- `CPP/` — read/write, static or DLL.
- `Rust/` — read/write archive access plus DDJ -> RGBA decoding (DXT1/3/5 and uncompressed 16/24/32bpp).
...
let mut pk2 = Pk2Archive::new();
pk2.open(Path::new("media.pk2"), OpenMode::ReadWrite)?;
pk2.make_folder("newdir\\sub")?;
pk2.write_file("newdir\\file.bin", b"data")?;
